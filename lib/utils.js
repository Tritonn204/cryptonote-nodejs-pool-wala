...
// Initialize a module-level variable for job counters
exports.localJobCounters = {};

/**
 * Generate a unique job ID with optional pool-specific counters.
 * Wraps around after 4 bytes (0xFFFFFFFF).
 * @param {number} indexOfChildPool - (Optional) The pool index to use for tracking local job IDs.
 * @returns {string} A unique job ID as a 4-byte hex string.
 */
exports.generateJobID = function (indexOfChildPool) {
  // Initialize the counter for the pool if it doesn't exist
  if (!exports.localJobCounters[indexOfChildPool]) {
    exports.localJobCounters[indexOfChildPool] = 0;
  }

  // Increment the counter and apply a 4-byte mask to ensure wraparound
  exports.localJobCounters[indexOfChildPool] =
    (exports.localJobCounters[indexOfChildPool] + 1) & 0xFFFFFFFF;

  // Return the job ID as a 4-byte hex string
  return exports.localJobCounters[indexOfChildPool].toString(16).padStart(8, "0");
};

exports.getCurrentJobID = function (indexOfChildPool) {
  if (!exports.localJobCounters) {
    exports.localJobCounters = {};
  }

  // Return the current job ID for the pool, or "00000000" if it hasn't been initialized
  const currentJobID = exports.localJobCounters[indexOfChildPool] || 0;

  // Format as a 4-byte hex string
  return currentJobID.toString(16).padStart(8, "0");
};

exports.extraNonceCounter = 0;

/**
 * Generate the next extra nonce as a 3-byte overflow counter.
 * @returns {number} A 3-byte overflow counter (0-16777215).
 */
exports.generateExtraNonce = function () {
  // Increment the counter and apply a 3-byte mask to ensure overflow wraps back to 0
  exports.extraNonceCounter = (exports.extraNonceCounter + 1) & 0xFFFFFF;

  // Return the updated counter
  return exports.extraNonceCounter;
}
