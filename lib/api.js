...
function collectStats() {
  ...
  poolPipe
    ...
    .zRangeWithScores(`${config.coin}:blocks:matured`, 0, config.api.blocks - 1, 'WITHSCORES', { REV: true })
    .hGetAll(`${config.coin}:scores:prop:roundCurrent`)
    .hGetAll(`${config.coin}:stats`)
    .zRangeWithScores(`${config.coin}:payments:all`, 0, config.api.payments - 1, 'WITHSCORES', { REV: true })
