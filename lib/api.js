...
function collectStats () {
  ...
  poolPipe
    ...
    .zRangeWithScores(`${config.coin}:blocks:candidates`, 0, -1, 'WITHSCORES')
    .zRangeWithScores(`${config.coin}:blocks:matured`, 0, config.api.blocks - 1, 'WITHSCORES', 'REV')
    .hGetAll(`${config.coin}:scores:prop:roundCurrent`)
    .hGetAll(`${config.coin}:stats`)
    .zRangeWithScores(`${config.coin}:payments:all`, 0, config.api.payments - 1, 'WITHSCORES', 'REV')
    ...
    pool: function (callback) {
      poolPipe.exec()
        ...
        redisClient.zRangeWithScores(config.coin + ':blocks:matured', 0, -1, 'REV')
        .then((result) => {
...
function handleMinerStats (urlParts, response) {
    ...
    redisClient
      .multi()
      .hGetAll(`${config.coin}:workers:${address}`)
      .zRangeWithScores(`${config.coin}:payments:${address}`, 0, config.api.payments - 1, 'REV')
      .keys(`${config.coin}:unique_workers:${address}~*`)
      .get(`${config.coin}:charts:hashrate:${address}`)
      .exec()
      .then((replies) => {
        ...
        const workersData = replies[2].map((key) => {
        ...
        charts.getUserChartsData(address, paymentsData, (error, chartsData) => {
          if (error) {
            ...
            response.end(JSON.stringify({ error: 'Chart data error' }));
            return;
          }

          const redisCommands = workersData.flatMap((worker) => [
          ...
          redisClient
            ...
            .catch((error) => {
              response.writeHead(500, { 'Content-Type': 'application/json' });
              response.end(JSON.stringify({ error: 'Redis worker data error', details: error.message }));
            });
        });
      })
      .catch((error) => {
        response.writeHead(500, { 'Content-Type': 'application/json' });
        response.end(JSON.stringify({ error: 'Redis multi error', details: error.message }));
      });
  }
}
...
function startRpcMonitoring (module, method, interval) {
    ...
    let response = {};    
    try {
      if (module == 'getblockcount') {
        response = {
          "msg": "not implemented"
        };
      } else if (module == 'getbalance') {
        rpcClient.getBalanceByAddress({address: config.poolServer.poolAddress})
          .then((reply) => {
            reponse = reply;
          });
      }
