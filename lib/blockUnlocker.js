          ...
          .then((result) => {
            ...
            redisClient
              ...
              .then((daaScore) => {
                ...
                for (const idx in result.block.transactions) {
                  const tx_base = result.block.transactions[idx];
                ...
                log('error', logSystem, 'Error calculating reward for block %s - %j', [block.hash, error]);
                mapCback();
                return;
              });
          })
      ...
      for (idx in blocks) {
        ...
        if (block.workerScores) {
          ...
          if (block.rewardType === 'solo') {
            ...
            Object.keys(block.workerScores).forEach((worker) => {
              const scaledWorkerPercent = BigInt(Math.round((block.workerScores[worker] / totalScore) * SCALE)); // Scale worker percent
              ...
              log('info', logSystem, 'PROP Block %s payment to %s for %s% of total block score: %s', [
                block.height.toString(10),
                worker,
                ((scaledWorkerPercent / bigScale) * 100n).toString(10),
      ...
      multi.exec()
        .then((replies) => {
          if (!replies || replies.length === 0) {
            log('info', logSystem, 'No commands executed for unlocked blocks');
            callback(true);
            return;
          }

          // Check for individual errors in replies
          const failedCommands = replies.filter((reply) => reply instanceof Error);
          if (failedCommands.length > 0) {
            log('error', logSystem, 'Some commands failed in the Redis pipeline:');
            failedCommands.forEach((error, idx) => {
              log('error', logSystem, `Error in command ${idx}: ${error.message}`);
            });
            callback(true);
            return;
          }
