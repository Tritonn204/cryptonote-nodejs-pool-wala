...
function runInterval () {
    ...
    // TODO: scan new blocks for relegated coinbase TXs to the pool address, to catch cases where
    // the DAG pays the coinbase tx for a red block in a subsequent blue block, which often happens
    // as opposed to true orphaning
      ...
      async.filter(blocks, function (block, mapCback) {
        ...
        rpcClient.getBlock({ hash: block.hash })
          .then(result => {
            ...
            // If the block's reward calculation depends on the miner's vout (output), handle that as well
            if (config.blockUnlocker.useFirstVout) {
              let vout = JSON.parse(result.json).miner_tx.vout;
              if (!vout.length) {
                log('error', logSystem, 'Error: tx at block %s has no vouts!', [blockHeight]);
                block.unlocked = false;
                mapCback();
                return;
              }
