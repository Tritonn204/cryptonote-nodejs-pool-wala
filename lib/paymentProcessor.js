...
function runInterval() {
  async.waterfall([
    ...
    function (balances, minPayoutLevel, callback) {
      ...
      async.filter(transferCommands, async function (transferCmd, cback) {
        let rpcRequest = transferCmd.rpc;

        // Assuming we are working with `rpcRequest.destinations` and `rpcRequest.fee` as the main input.
        let totalAmount = 0;
        let outputs = [];
        let entries = []; // UTXOs to be used

        // Collect destinations and sum up the amount
        rpcRequest.destinations.forEach(destination => {
          outputs.push({
        ...
        // Create a Generator to handle batch transactions
        let generator = new Generator({
          entries: entries,  // Should be your UTXOs (if available)
          outputs: outputs,  // Destinations for the batch
          priorityFee: amountToSompi(config.payments.transferFee), // Set priority fee
          changeAddress: transferCmd.changeAddress || global.privateKey.toKeypair().toAddress(config.networkId)  // Default change address if not provided
        });
        ...
        while (pending = await generator.next()) {
          try {
            // Sign the transaction using the environment private key
            await pending.sign([global.privateKey]);

            // Submit the transaction to the rpcClient
            let txid = await pending.submit(rpcClient);
            transactionIds.push(txid);  // Collect the transaction IDs

            let now = (timeOffset++) + Date.now() / 1000 | 0;
            let txHash = txid.replace('<', '').replace('>', '');  // Clean txHash format

            // Update Redis with transaction info
            transferCmd.redis.push(['zadd', config.coin + ':payments:all', now, [
              txHash,
              transferCmd.amount,
              transferCmd.rpc.fee,
              config.payments.ringSize || config.payments.mixin,
              rpcRequest.destinations.length
            ].join(':')]);

            let notify_miners_on_success = [];
            for (let i = 0; i < rpcRequest.destinations.length; i++) {
              let destination = rpcRequest.destinations[i];
              if (rpcRequest.payment_id) {
                destination.address += config.poolServer.paymentId.addressSeparator + rpcRequest.payment_id;
              }
            ...
            // Handle the Redis multi command
            redisClient.multi(transferCmd.redis)
              .exec(function (error, replies) {
                if (error) {
                  log('error', logSystem, 'Critical error: Payments sent but failing to update Redis. Double payouts may occur. %j', [error]);
                  log('error', logSystem, 'Double payments likely to be sent to %j', transferCmd.rpc.destinations);
                  cback(false);
                  return;
                }

                // Notify miners of successful payment
                for (let m in notify_miners_on_success) {
                  notify_miners.push(notify_miners_on_success[m]);
                }

                cback(true);  // Confirm successful transaction
              });
          } catch (err) {
            log('error', logSystem, 'Error while submitting transaction %s: %j', [pending.txid, err]);
            cback(false);
            return;
          }
        }
        ...
        log('info', logSystem, 'Payments processed: %d transactions, IDs: %j', [transactionIds.length, transactionIds]);

        // Ensure Redis cleanup if necessary (not shown here but may involve deletion or flagging entries)
