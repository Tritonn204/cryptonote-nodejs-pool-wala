...
Miner.prototype = {
  ...
  addJob: function (id) {
    ...
    newJob.difficulty = this.difficulty;
		...
		while (this.validJobs.length > 10)
...
async function recordShareData(miner, job, jobDiff, shareDiff, blockCandidate, hashHex, shareType, blockTemplate, pool) {
  try {
    ...
    if (workerName) {
      multi.zAdd(`${coin}:hashrate`, { score: dateNowSeconds, value: `${jobDiff}:${login}~${workerName}:${dateNow}:${rewardType}` });
...
function processShare (miner, job, blockTemplate, params) {
  ...
  let reversedNonceHex = paddedNonceHex.match(/.{2}/g).reverse().join("");
  let extraNonceBuffer = Buffer.from(reversedNonceHex, "hex");
  
  // Pass the reversed nonce to checkPow
  let powResult = powState.checkPow(BigInt(`0x${paddedNonceHex}`));

  const reserveOffset = templateComp.length - 8; // Calculate the offset for the last 8 bytes
  extraNonceBuffer.copy(templateComp, reserveOffset);
  ...
  if (powResult[0]) {
    ...
    //     );
    //     recordShareData(miner, job, powResult[1].toString(), true, blockFastHash, shareType, blockTemplate, null);
    //   }
    // });
  } else {
    ...
    if (powResult[1] < poolTarget) {
      recordShareData(miner, job, miner.difficulty, ((1n << 256n)-1n)/powResult[1], false, null, 'valid', blockTemplate, null);
    } else {
