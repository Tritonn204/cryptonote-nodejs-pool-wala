...
function bitsToDiff(bits) {
  ...
  if (mantissa & 0x800000) {
    throw new Error("Invalid bits field: mantissa should not have the sign bit set.");
  }

  const target = BigInt(mantissa) * (2n ** (8n * BigInt(exponent - 3)));
  return target;
}
...
class ShareRateLimiter {
  constructor() {
    ...
    this.WINDOW_SIZE = 1000; // 1 second
    this.MAX_SHARES = 20;    // max 20 shares per second
    this.BAN_TIME = 30000;   // 30 second ban
  }

  isRateLimited(minerKey) {
    const now = Date.now();
    if (!this.shareCounts.has(minerKey)) {
      this.shareCounts.set(minerKey, {
        shares: [{ timestamp: now }],
        bannedUntil: 0
      });
      return false;
    }

    const minerData = this.shareCounts.get(minerKey);

    // Check if currently banned
    if (minerData.bannedUntil > now) {
      return true;
    }

    // Clean old shares
    minerData.shares = minerData.shares.filter(share =>
      share.timestamp > now - this.WINDOW_SIZE
    );

    // Add new share
    minerData.shares.push({ timestamp: now });

    // Check if rate limit exceeded
    if (minerData.shares.length > this.MAX_SHARES) {
      minerData.bannedUntil = now + this.BAN_TIME;
      redisClient.hIncrBy(
        `${config.coin}:rate_limit_stats`,
        miner.login,
        1
      );
      log('warn', logSystem, 'Miner %s rate limited for excessive share submissions', [minerKey]);
      return true;
    }
  ...
  clearMiner(minerKey) {
    this.shareCounts.delete(minerKey);
  }
}
...
class BlockTemplateProcessor {
  ...
  updateTemplates(indexOfChildPool, blockData, buffer) {
    ...
    while (validBlockTemplates[indexOfChildPool].length > 2) {
...
function shouldReplaceChildBlockTemplate(poolIndex, block) {
  ...
  return (
    !currentTemplate ||
    block.header.daaScore > currentTemplate.block.header.daaScore ||
...
Miner.prototype = {
  retarget: function (now) {
    ...
    newDiff = Math.ceil(newDiff*100) / 100; // round up difficulty to nearest 2 decimal places
  ...
  addJob: function (id) {
    ...
    let newJob = {
      id: id,
      height: blockTemplate.block.header.daaScore,
  ...
  submitShare: function () {
    const now = Date.now();
    const delta = now - this.lastShareTimestamp;
    this.lastShareTimestamp = now;

    // Keep last 10 deltas
    this.shareTimeDeltas.push(delta);
    if (this.shareTimeDeltas.length > 20) {
      this.shareTimeDeltas.shift();
    }

    // Check for suspiciously consistent timing
    if (this.shareTimeDeltas.length >= 5) {
      const variance = calculateVariance(this.shareTimeDeltas);
      if (variance < 10) { // Suspiciously consistent timing
        log('warn', logSystem, 'Suspicious share timing detected from %s', [this.login]);
        return false;
      }
    }
...
function handleMinerMethod(method, params, socket, portData, sendReply, pushMessage) {
  ...
  switch (method) {
    ...
    case 'mining.submit':
      ...
      if (shareRateLimiter.isRateLimited(subscriptionKey)) {
        sendReply('Share submission rate limit exceeded');
        return;
      }
      ...
      if (!miner.submitShare()) {
        sendReply('Suspicious Share Timing Detected');
        return;
      }
      ...
      let isJobBlock = function (b) {
        return b.block.header.daaScore === job.height && job.childHeight === (
          b.childBlockTemplate ? b.childBlockTemplate.block.header.daaScore : undefined);
      };

      const isTip = isJobBlock(currentBlockTemplate[miner.activeChildPool]);
      let blockTemplate = isTip ? currentBlockTemplate[miner.activeChildPool] : validBlockTemplates[miner.activeChildPool].filter(isJobBlock)[0]; 
      if (!blockTemplate) {
        sendReply(`Stale share`);
        return;
      }

      let shareAccepted = processShare(miner, job, blockTemplate, params, isJobBlock(currentBlockTemplate[miner.activeChildPool]));
...
function removeConnectedWorker(miner, reason) {
  ...
  shareRateLimiter.clearMiner(subscriptionKey);
...
//   multi.rename(`${coin}:scores:prop:roundCurrent`, `${coin}:scores:prop:round${debugBlockJSON.height}`);
//   multi.rename(`${coin}:shares_actual:prop:roundCurrent`, `${coin}:shares_actual:prop:round${debugBlockJSON.height}`);
//   // if (rewardType === 'prop') {
//     multi.hGetAll(`${coin}:scores:prop:round${debugBlockJSON.height}`);
//     multi.hGetAll(`${coin}:shares_actual:prop:round${debugBlockJSON.height}`);
//   // }
...
async function recordShareData(miner, job, jobDiff, shareDiff, blockCandidate, hashHex, shareType, blockTemplate, pool) {
  try {
    ...
    if (blockCandidate) {
      ...
      multi.rename(`${coin}:scores:prop:roundCurrent`, `${coin}:scores:prop:round${jobHeight}`);
      multi.rename(`${coin}:shares_actual:prop:roundCurrent`, `${coin}:shares_actual:prop:round${jobHeight}`);
      if (rewardType === 'prop') {
        multi.hGetAll(`${coin}:scores:prop:round${jobHeight}`);
        multi.hGetAll(`${coin}:shares_actual:prop:round${jobHeight}`);
      }
    }
    ...
    if (blockCandidate) {
      ...
      let difficulty = ((1n << 256n) - 1n) / bitsToDiff(blockTemplate.header.bits);
      ...
      if (workerName) {
        await redisClient.rPush(`${coin}:unique_worker_blocks:${login}~${workerName}`, [jobHeight.toString(10), serialized].join('||'));
      }
 ...
 **/
function processShare(miner, job, blockTemplate, params, isJobBlock) {
  ...
  if (isJobBlock && powResult[0]) {
    ...
    let result = rpcClient.submitBlock({
      ...
      .then((res) => {
        ...
        if (result.report.type === 'success') {
          ...
          recordShareData(miner, job, miner.difficulty, ((1n << 256n) - 1n) / poolTarget, true, blockID, 'valid', blockTemplate.block, null);
        } else {
    ...
    if (powResult[1] < poolTarget) {
      recordShareData(miner, job, miner.difficulty, ((1n << 256n) - 1n) / poolTarget, false, null, 'valid', blockTemplate.block, null);
    } else {
  ...
  async.each(config.poolServer.ports, function (portData, cback) {
    ...
    let socketResponder = function (socket) {
      ...
      socket.on('data', function (d) {
        const now = Date.now();
        if (now - lastMessageTime < 1000) { // Within 1 second
          messageCount++;
          if (messageCount > 50) { // More than 50 messages per second
