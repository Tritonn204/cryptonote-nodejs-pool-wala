  ...
  const remotePort = socket.remotePort;
    ...
    case 'mining.authorize': {
      ...
      let miner = sub.workers[nonceExtra];
        ...
        console.log("available jobs:", miner.validJobs);
...
function processShare (miner, job, blockTemplate, params) {
  ...
  console.log("submitted template:", templateComp.toString("hex"));
  ...
  if (powResult[0]) {
    ...
    if (powResult[1] < poolTarget) {
      recordShareData(miner, job, powResult[1].toString(), false, null, 'valid', blockTemplate, null);
    } else {
