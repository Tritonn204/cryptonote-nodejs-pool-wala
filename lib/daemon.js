...
const getTemplate = async () => {
  try {
    ...
    const newDaaScore = blockTemplate.block.header.daaScore;

    // Fetch the current DAA score from Redis
    const currentDaaScoreStr = await redisClient.get(`${config.coin}:daaScore`);
    const currentDaaScore = currentDaaScoreStr ? BigInt(currentDaaScoreStr) : 0;

    if (newDaaScore > currentDaaScore) {
      await redisClient.set(`${config.coin}:daaScore`, newDaaScore.toString(10));
      log('info', logSystem, `DAA score updated to ${newDaaScore}`);
    } else {
      log('info', logSystem, `DAA score ${newDaaScore} not greater than current ${currentDaaScore}. No update performed.`);
    }
  } catch (error) {
